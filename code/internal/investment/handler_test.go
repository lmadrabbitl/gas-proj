...
func TestCreateOperationForwardsPayload(t *testing.T) {
	...
	c.Set("userID", userID)
	c.Request = httptest.NewRequest(http.MethodPost, "/investments/operations", bytes.NewBufferString(`{"asset_code":"mxrf11","brokerage_account_code":"btg-invest","investment_account_code":"investimentos-b3","operation_type":"buy","date":"2026-06-20T00:00:00Z","quantity":10,"unit_price":1234,"fee_amount":10,"notes":"teste"}`))
...
func TestCreateOperationParsesDate(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(w)
	c.Set("userID", uuid.New())
	c.Request = httptest.NewRequest(http.MethodPost, "/investments/operations", bytes.NewBufferString(`{"asset_code":"mxrf11","brokerage_account_code":"btg-invest","investment_account_code":"investimentos-b3","operation_type":"buy","date":"2026-06-20T00:00:00Z","quantity":1,"unit_price":100,"fee_amount":0}`))
	c.Request.Header.Set("Content-Type", "application/json")
...
func TestCreateBulkOperationsForwardsRows(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(w)
	c.Set("userID", userID)
	c.Request = httptest.NewRequest(http.MethodPost, "/investments/operations/bulk", bytes.NewBufferString(`{"operations":[{"asset_code":"mxrf11","brokerage_account_code":"btg-invest","investment_account_code":"investimentos-b3","operation_type":"buy","date":"2026-06-20T00:00:00Z","quantity":10,"unit_price":1234,"total_fee_amount":100}]}`))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	if got.Operations[0].BrokerageAccountCode != "btg-invest" || got.Operations[0].InvestmentAccountCode != "investimentos-b3" || got.Operations[0].OperationType != OperationTypeBuy || got.Operations[0].TotalFeeAmount != 100 {
